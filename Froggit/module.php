    ...
    protected function ProcessHookData()
    {
        ...
        foreach ($_POST as $key => $value) {
            ...
            switch ($key) {
                ...
                case 'runtime':
                    ...
                    if($ID && ($this->GetValue($key) != $uptime || $SaveAllValues)) {
                        $this->SetValue($key, intval($uptime));
                    }
                    break;
                ...
                case 'windspdmph_avg10m':
                    ...
                    if($ID && ($this->GetValue($key) != $wind->windspeed || $SaveAllValues)) {
                        $this->SetValue($key, $wind->windspeed);
                    }
                    break;
                ...
                case 'tf_co2':
                    ...
                    if($IgnoreImprobableValues && $value <= -1000) {
                        ...
                        if(is_numeric(substr($key, -1))) {
                            ...
                            $sensor = $this->Translate('Outdoor sensor');
                        } elseif($key == 'tempinf') {
                            $sensor = $this->Translate('Indoor sensor');
                        }
                        $ID = $this->VariableCreate('float', $key, $name . ' (' . $sensor . ')', $profile, 100 + $pos);
                        if($ID && ($this->GetValue($key) != $temp || $SaveAllValues)) {
                ...
                case (substr($key, 0, 11) == 'soil_ec_hum' && strlen($key) == 12):
                ...
                case 'vpd':
                    $ID = $this->VariableCreate('float', $key, "VPD", '', 705);
                    if($ID && ($this->GetValue($key) != $value || $SaveAllValues)) {
                        $this->SetValue($key, $value);
                    }
                    break;
                ...
                case 'baromrelin':
                    ...
                    if($ID && ($this->GetValue($key) != $pressure || $SaveAllValues)) {
                        $this->SetValue($key, $pressure);
                    }
                    break;
                ...
                case (strpos($key, 'rain') !== false):
                    ...
                    if($ID && ($this->GetValue($key) != $rain || $SaveAllValues)) {
                        $this->SetValue($key, $rain);
                    }
                    break;
        ...
        if ($this->ReadPropertyBoolean("DewPoint") && array_key_exists('tempf', $_POST) && array_key_exists('humidity', $_POST)) {
            ...
            if($ID && ($this->GetValue($key) != $dewpoint || $SaveAllValues)) {
                $this->SetValue($key, $dewpoint);
            }
        } elseif ($Debug) {
            $this->SendDebug("Dew Point Calculation", "inactive or missing data", 0);
        }
        ...
        if ($this->ReadPropertyBoolean("DewPoint") && array_key_exists('tempf', $_POST) && array_key_exists('windspeedmph', $_POST)) {
            ...
            if ($temp <= 10 && $wind->windspeed > 5) {
                $windchill = 13.12 + 0.6215 * $temp - 11.37 * pow($wind->windspeed, 0.16) + 0.3965 * $temp * pow($wind->windspeed, 0.16);
            } else {
                $windchill = $temp;
            }
            $ID = $this->VariableCreate('float', $key, 'Windchill', $profile, 111);
            if($ID && ($this->GetValue($key) != $windchill || $SaveAllValues)) {
                $this->SetValue($key, $windchill);
            }
        } elseif ($Debug) {
            $this->SendDebug("Windchill Calculation", "inactive or missing data", 0);
        }
    }
        ...
        if ($this->ReadPropertyBoolean("CreateVariables")) {
            switch ($Type) {
                ...
                default:
                    $return = false;
                    if ($Debug) {
                        $this->SendDebug(__FUNCTION__, "The Variable typ:  " . $type . " don't exist", 0);
                    }
            }
        } else {
            ...
            if (!$return) {
                if ($Debug) {
                    $this->SendDebug(__FUNCTION__, "The Variable " . $this->Translate($Name) . " with Key: " . $Ident . " were not created", 0);
                }
            }
        }
        if ($Debug > 1) {
            $this->SendDebug(__FUNCTION__, "Variable ID: " . $return . " for Name: " . $Name, 0);
        }
        return $return;
    }
}
